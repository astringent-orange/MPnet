...
class DLASeg(nn.Module):
    ...
    DLASeg 主体结构：
    1. 时序下采样(ConvGRUNet)
    2. 静态+时序主干特征提取(backbone)
    3. 掩码注意力增强(MaskProp)
    4. 多尺度上采样融合(DLAUp/IDAUp)
    5. 时序上采样(DeConvGRUNet)
    6. 掩码生成(mask_layer)
    7. 多任务输出头
    forward输入: img_input (B, N, 3, H, W)
    forward输出: [temp_feat, ret]，其中ret为各任务输出
    """
    def __init__(self, heads, final_kernel,
                  head_conv, out_channel=0):
        # self.backbone: DLA34主干，输入单帧+时序特征，输出多尺度特征
        # self.conv_gru: 时序下采样，输入(B, N, 3, H, W)，输出(B, N, 16, H, W)
        # self.mask_prop1/2: 掩码注意力增强
        # self.dla_up: 多尺度特征上采样融合
        # self.ida_up: 层间特征融合
        # self.deconv_gru: 时序上采样，输入(B, N, 32, H, W)，输出(B, N-1, 32, H, W)和(B, 16, H, W)
        # self.mask_layer: 生成掩码，输入(B, 32, H, W)，输出(B, 1, H, W)
        # self.heads: 多任务输出头
    ...
    def forward(self, img_input, training=True, vid=None):
        """
        主流程：
        1. 时序下采样(ConvGRUNet)：img_input -> c0_gru (B, N, 16, H, W)
        2. 逐帧静态+时序特征提取(backbone)，掩码注意力增强(MaskProp)
        3. 多尺度上采样融合(DLAUp/IDAUp)
